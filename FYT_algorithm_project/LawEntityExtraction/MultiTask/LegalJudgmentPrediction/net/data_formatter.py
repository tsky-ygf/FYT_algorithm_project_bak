...
from LawEntityExtraction.MultiTask.LegalJudgmentPrediction.net.loader import accusation_dict, accusation_list, law_dict, law_list
...
def parse(data, config, tokenizer):
    label_list = config.get("data", "type_of_label").replace(" ", "").split(",")
    label = []
    for x in label_list:
        if x == "crit":
            label.append(analyze_crit(data["meta"]["accusation"], config))
        if x == "law":
            label.append(analyze_law(data["meta"]["relevant_articles"], config))
        if x == "time":
            label.append(analyze_time(data["meta"]["term_of_imprisonment"], config))

    inputs = tokenizer(data["fact"],
                       add_special_tokens=True,
                       max_length=int(config.get("net", "maxlength")),
                       padding="max_length",
                       truncation=True,
                       return_offsets_mapping=False,
                       return_tensors="pt")
    # vector, len_vec = generate_vector(data["content"], config, transformer)
    return inputs, torch.cat(label)
