...
class ContractTypeResult(BaseModel):
    type_id: str = Field(description="合同类型的id")
    contract_type: str = Field(description="合同类型")


class ContractTypesResponse(BaseModel):
    result: list[ContractTypeResult]


@app.get('/fyt/ai/v1.0.0/get_contract_type', summary="获取能审核的合同类型", response_model=ContractTypesResponse)
...
class UserResult(BaseModel):
    id: str = Field(description="甲乙方代表id")
    standpoint: str = Field(description="甲乙方代表")


class UserResponse(BaseModel):
    result: list[UserResult]


@app.get('/fyt/ai/v1.0.0/get_user_standpoint', summary="获取甲乙方代表", response_model=UserResponse)
...
class ContractInput(BaseModel):
    contract_type_id: str = Field(default="laowu", description="合同类型")
    usr: str = Field(default="party_a", description="甲乙方代表")
    contract_content: str = Field(default="劳务雇佣合同甲⽅（⽤⼈单位）：太极信息有限公司地址：浙江省杭州市滨江区物联网大街222号电话......", description="合同类型")
...
class ContractResult(BaseModel):
    review_point: str = Field(description="审核点")
    show_name: str = Field(description="展示名称")
    review_result: str = Field(description="审核结果")
    review_content: str = Field(description="审核内容")
    review_content_start: str = Field(description="审核内容在文档中的起始坐标(多个坐标时，用#拼接)")
    review_content_end: str = Field(description="审核内容在文档中的末尾坐标(多个坐标时，用#拼接)")
    legal_advice: str = Field(description="法律建议")
    legal_basis: str = Field(description="法律依据")
    risk_level: str = Field(description="风险等级")
    risk_point: str = Field(description="风险点")


class ContractResponse(BaseModel):
    result: list[ContractResult]


@app.post("/fyt/ai/v1.0.0/get_contract_review_result", summary="获取合同审核结果", response_model=ContractResponse)
async def _get_contract_review_result(contract_input: ContractInput):
    ...
    result = get_contract_review_result(content=contract_input.contract_content, mode="text",
                                        ...
                                        usr=contract_input.usr)
    return {"result": result}


class FileLinkInput(BaseModel):
    file_path: str = Field(
        default="https://nblh-fyt.oss-cn-hangzhou.aliyuncs.com/fyt/20220916/55712bdc-694a-438f-bcb0-1f5b66dd9bb5.docx",
        description="需要审核的文件的链接")
...
@app.post("/fyt/ai/v1.0.0/get_text_from_file_link_path", summary="从文件链接获取文件文本内容", response_model=TextResponse)
