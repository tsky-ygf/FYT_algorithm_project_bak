...
def relevant_laws_side_review():
    ...
    if run:
        url = "http://127.0.0.1:8132/search_laws"
        body = {
            "query": text,
            "filter_conditions": {
                "types_of_law": [
...
def similar_case_retrieval_review():
    # 初始化
    logger = Logger(name="law-lib", level="debug").logger
    url_similar_case = "http://127.0.0.1:8132/top_k_similar_narrative"
    url_jfType = "http://127.0.0.1:8133/get_civil_problem_summary"
    url_suqiu = "http://0.0.0.0:8133/get_claim_list_by_problem_id"
