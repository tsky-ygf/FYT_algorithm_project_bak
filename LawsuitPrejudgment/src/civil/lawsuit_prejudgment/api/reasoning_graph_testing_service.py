...
def hello_world():
    try:
        ...
        if in_json is not None:
            ...
            repeated_question_management = in_dict.get('repeated_question_management')
            ...
            result_dict = predict_fn(problem, claim_list, fact, question_answers, factor_sentence_list, True, repeated_question_management)
            ...
            repeated_question_management = result_dict['repeated_question_management']  # 匹配到短语的列表
            ...
            return json.dumps(
                {'question_asked': question_asked, 'question_next': question_next, "question_type": question_type,
                 "factor_sentence_list": factor_sentence_list, "repeated_question_management": repeated_question_management, "debug_info": debug_info, "result": result,
...
@app.route('/reasoning_graph_testing/upload_knowledge', methods=["post"])
def upload_knowledge():
    return json.dumps({"msg": "Success", "status": 0}, ensure_ascii=False)


if __name__ == "__main__":
    app.run(host="0.0.0.0", port=6080, debug=True)  # , use_reloader=False)
