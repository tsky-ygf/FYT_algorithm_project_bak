...
class LogicTree(object):
    def __init__(self, problem, suqiu, debug=True):
        ...
        self.suqiu = suqiu
        self.debug = debug
        self.suqiu_advice = suqiu_advice_dict[problem+'_'+suqiu]
    ...
    def add_question_result(self, question, answers):
            ...
            for factor, answer in self.candidate_factor.items():
                ...
                for ans in answer.split('|'):
                    if ans in answers:
                        ...
                        add = True
                        self._set_factor_flag(factor, 1, question + ':' + ans)
                ...
                # if '以上都不是' in answers or '以上都没有' in answers:
                #     self._set_factor_flag(factor, -1, None)
                #     if factor in self.factor_exclusion and len(self.factor_exclusion[factor]) == 1:
                #         for f in self.factor_exclusion[factor]:
                #             self._set_factor_flag(f, 1, None)
            self.match_flag = True
            return factors
        ...
        for factor in self.question_factor[question]:
            ...
            for answer in self.factor_answer[factor].split('|'):
                if answer in answers:
                    factors.append(factor)
                    self._set_factor_flag(factor, 1, question + ':' + answer)
                    add = True
                    if factor in self.factor_exclusion:
                        for f in self.factor_exclusion[factor]:
    ...
    def add_suqiu_result(self, suqiu, flag):
        """
        多诉求情形将其他诉求的支持结果添加到树中
        :param suqiu:
        :param flag:
        :return:
        """
        if '诉求支持_' + suqiu in self.factor_flag:
            print(suqiu, flag)
            self.factor_flag['诉求支持_' + suqiu] = flag
        if '诉求不支持_' + suqiu in self.factor_flag:
            print(suqiu, flag)
            self.factor_flag['诉求不支持_' + suqiu] = -flag
    ...
    def get_next_question(self):
            ...
            if if_ask:
                ...
                for node in temp:
                    ...
                    if node.question in questions:
                        questions[node.question] = [min(len(temp), questions[node.question][0]),
                                                    max(weight, questions[node.question][1])]
                    else:
                        questions[node.question] = [len(temp), weight]
        ...
        if len(questions) > 0:
            question = sorted(questions.items(), key=lambda x: x[1][0] * 50000 - x[1][1])[0][0]
            return question
    ...
    def get_support_proof(self, path):
        ...
        for p in path[1:]:
            ...
            if factor in self.factor_proof:
                proof.append('%d. %s等，用以证明%s的事实；' % (len(proof) + 1, self.factor_proof[factor], factor))
        for path in self.paths:
            if path[0].father.father.name not in ['大前提']:
                continue
            for p in path[1:]:
                factor = p.name.replace('_', '')
                if factor in self.factor_proof:
    ...
    def get_logic_result(self, probability):
        if self.logic_result[0] == '不满足前提':
            ...
            if probability < 0.5:
                if len(self.support_proof)>0:
                    reason += '\n需要注意的是，基于裁判文书中相似案例的分析结果显示，您的诉求支持率较低，这是由于很多案件在庭审过程中【举证不足】，导致法院驳回诉讼请求。'
                    probability = 0.3 + probability / 2.5
                else:
                    probability = 0.5 + probability / 2
            if len(self.support_proof)>0:
                proof = '根据您所描述的事实，建议搜集以下能够证明您所描述事实的证据：\n%s' % ('\n'.join(self.support_proof)[:-1] + '。')
            else:
                proof = ''
            advice = re.sub('^法律建议[，：:,]{0,1}', '', self.logic_result[2])
        else:
            result = -1
            reason = self.logic_result[1]
            probability = 0.5 + (probability-0.5) / 4
            if probability > 0.5:
            ...
            advice = re.sub('^法律建议[，：:,]{0,1}', '', self.logic_result[2])
        if not self.debug:
            np.random.seed(sum(v for v in self.factor_flag.values() if v ==1))
            probability = min(probability, 0.85+np.random.random()*0.05)  # 最后输出不会超过0.93
            probability = max(probability, 0.1+np.random.random()*0.05)

            reason = self.text_underline(reason)
            proof = self.text_underline(proof)
            advice = self.text_underline(advice)
        return result, probability, reason, proof, advice

    def print_logic_result(self, probability):
        """
        打印评估理由
        :return:
        """
        result, probability, reason, proof, advice = self.get_logic_result(probability)
        print('支持率: %s' % (probability))
        print('理由: %s' % (reason))
        print('证据: %s' % (proof))
        print('法律建议: %s' % (advice))
        return result
...
if __name__=='__main__':
    tree = LogicTree('婚姻家庭', '遗产继承')
    print('诉求不支持_确认遗嘱有效' in tree.factor_flag)
    tree.print_paths()
