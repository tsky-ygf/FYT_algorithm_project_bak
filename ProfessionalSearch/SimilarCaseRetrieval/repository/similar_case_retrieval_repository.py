...
def get_criminal_law_documents_by_id_list(id_list: List[str]) -> List[Dict]:
    # 打开数据库连接
    db = pymysql.connect(
        host="rm-8vbhaq1jlv38l7qeyoo.mysql.zhangbei.rds.aliyuncs.com",
    ...
    try:
        ...
        # 执行SQL语句
        cursor.execute(
            "SELECT uq_id, htmlContent FROM judgment_xingshi_data WHERE uq_id in (%s)"
            % format_strings,
            tuple(id_list),
        )
        # 获取所有记录列表
        fetched_data = cursor.fetchall()
        law_documents = [
            {"doc_id": row[0], "html_content": row[1]} for row in fetched_data
        ]
    except:
        logging.error("Error: unable to fetch data")
        law_documents = []
...
def get_civil_law_documents_by_id_list(id_list: List[str], table_name) -> List[Dict]:
    ...
    db = pymysql.connect(
        ...
        user="root",
        password="Nblh@2022",
        database="test_document_227_data",
    )
...
def get_law_documents_by_id_list(id_list: List[str], table_name) -> List[Dict]:
    # 打开数据库连接
    db = pymysql.connect(
        host="172.19.82.227",
        user="root",
