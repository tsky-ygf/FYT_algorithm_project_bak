...
def get_civil_law_documents_by_id_list(id_list: List[str], table_name) -> List[Dict]:
    ...
    try:
        format_strings = ','.join(['%s'] * len(id_list))
        # 执行SQL语句
        cursor.execute("SELECT uq_id, content FROM " + table_name + " WHERE uq_id in (%s)" % format_strings,
                       tuple(id_list))
        # 获取所有记录列表
        fetched_data = cursor.fetchall()
        law_documents = [{
...
def get_law_documents_by_id_list(id_list: List[str], table_name) -> List[Dict]:
    ...
    try:
        format_strings = ','.join(['%s'] * len(id_list))
        # 执行SQL语句
        cursor.execute("SELECT uq_id, source FROM " + table_name + " WHERE uq_id in (%s)" % format_strings,
                       tuple(id_list))
        # 获取所有记录列表
        fetched_data = cursor.fetchall()
        law_documents = [{
