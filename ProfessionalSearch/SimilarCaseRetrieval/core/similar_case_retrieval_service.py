...
@Desc    : None
"""
from ProfessionalSearch.SimilarCaseRetrieval.repository import (
    similar_case_retrieval_repository as repository,
)
...
def get_civil_law_document(doc_id):
    law_documents = repository.get_civil_law_documents_by_id_list(
        [doc_id], "linux_minshi_document_03"
    )
    if not law_documents:
        law_documents = repository.get_civil_law_documents_by_id_list(
            [doc_id], "linux_minshi_document_05"
        )
    return law_documents[0] if law_documents else None
