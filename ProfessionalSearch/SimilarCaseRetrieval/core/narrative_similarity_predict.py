...
def predict_fn(fact, problem='', claim_list=[]):
    ...
    print("predict_fn.fact:" + fact + ";problem:" + problem + ";claim_list:" + str(claim_list))
    logging.info("predict_fn.fact:" + fact + ";problem:" + problem + ";claim_list:" + str(claim_list))
    ...
    if problem == '' and claim_list == []:  # 如果问题类型和诉求都为空的话，那么先做意图识别
        ...
        case_reason_confidence = str(problem_suqiu_type_dict.get('case_reason_confidence', ''))
        claim_list = problem_suqiu_type_dict.get('appeal', [])
        logging.info("case_reason:{}, appeal:{}, confidence:{},".format(problem, str(claim_list), case_reason_confidence))
        ...
        logging.info("problem:{}, appeal:{}, confidence:{},".format(problem, str(claim_list), case_reason_confidence))

    # 2. 召回
    t1 = time.time()
    candidate_list, tags = search_similar_case(fact, is_consult_flag, problem=problem, claim_list=claim_list)
    ...
    t2 = time.time()
    print('predict_fn.1.time spent for recall candidate list and compute similairity of questions:', (t2 - t1))
...
def candidate_list_q2q_rank(original_question, candidate_list):
    ...
    logging.info("narrative_similarity_predict.candidate_list_q2q_rank." + "original_question.filtered:" + original_question)
    ...
    for i, element in enumerate(candidate_list):
        doc_id, problem_type, suqiu_type, suqiu_label, suqing_sentences, chaming, benyuan_renwei, tags = element
        candidate = pseg_txt(chaming).replace(" ", "")  # 过滤一部分数据
        print("narrative_similarity_predict.candidate_list_q2q_rank.", i, "candidate:", candidate)
        logging.info("narrative_similarity_predict.candidate_list_q2q_rank." + str(i) + "candidate.filtered:" + candidate)
        ...
        candidate_list_new.append((doc_id, cos_i, win_los, problem_type, suqiu_type, tags))

    candidate_list_new = sorted(candidate_list_new, key=lambda element: element[1], reverse=True)  # 排序
    logging.info("narrative_similarity_predict.candidate_list_q2q_rank.candidate_list_new:" + str(candidate_list_new))
    print("narrative_similarity_predict.candidate_list_q2q_rank.candidate_list_q2q_rank:" + str(candidate_list_new))
...
def rank_candiate_list_v01(question_original, similiar_question_list):
    ...
    # TODO add tags
    emb_query, query_filtered = compute_sentence_embedding(question_pseg, vocab_dict, vcoab_embedding, idfs_dict, tfidf_vectorizer, tfidf_vocab_dict)
    ...
    for element in similiar_question_list:
        ...
        emb_question, _ = compute_sentence_embedding(candidate_sentences, vocab_dict, vcoab_embedding, idfs_dict, tfidf_vectorizer, tfidf_vocab_dict)
        # 候选问题计算句子向量
        cos_i = cosine_similiarity(emb_query, emb_question)
        win_los = 1 if '1' in suqiu_label else 0  # TODO 有一个诉求得到支持，暂时任务就是支持的。
        similiar_list.append((doc_id, as_num(cos_i), win_los, problem_type, suqiu_type, tags))
...
def search_similar_case(question_original, is_consult_flag, problem='', claim_list=[], top_k=15):
    """
    ...
    tags2 = jieba.analyse.textrank(question_original, topK=top_k, withWeight=False, allowPOS=('ns', 'n', 'vn', 'v'))
    tags = list(set(tags1).intersection(set(tags2)))
    tags = " ".join(tags)
    logging.info('search_similar_case.question_short:' + question_short + ";tags:" + tags + ";quesiton:" + question)

    question_last = question[-30:]
    query_search = " ".join(tags) + " " + question_last + " " + question_short  # query_search包括关键词、后面的比较重要的部分、过滤后的数据
    print("")
    # claim_list="x1,x2"
    ...
    if len(suqiu_list) > 0:
        ...
        problem = '劳动纠纷'
    suqiu_type = '' if len(claim_list) <= 0 else " ".join(claim_list)  # 如果诉求类型列表为空，则搜索空，否则排序后搜索

    logging.info("search_similar_case.problem:" + problem + ";suqiu_type:" + suqiu_type + "; original.claim_list:" + str(claim_list))
    print("search_similar_case.problem:" + problem + ";suqiu_type:" + suqiu_type + "; original.claim_list:" + str(claim_list))
    ...
    res = es.search(index=ES_IDX_NAME, size=search_size, body=body)  # res = es.search(index="test-index", body={"query": {"match_all": {}}})
    ...
    for hit in res['hits']['hits']:
        ...
        if count < NUM_MAX_CANDIDATES:
            print(count, '--->', doc_id, ";problem_type:", problem_type, ";suqiu_type:", suqiu_type, ";suqiu_label:", suqiu_label, ";tags_:", tags_)
            logging.info(str(doc_id) + ";problem_type:" + problem_type + ";suqiu_type:" + suqiu_type + ";suqiu_label:" + suqiu_label + ";tags_:" + tags_)
            print(count, "suqing_sentences:", suqing_sentences, ";chaming:", chaming, ";benyuan_renwei:", benyuan_renwei)
            logging.info("suqing_sentences:" + suqing_sentences + ";chaming:" + chaming + ";benyuan_renwei:" + benyuan_renwei)
            ...
            if problem != '' and len(problem) > 0:
                if problem_type == problem:
                    if len(claim_list)==0 or suqiu_type in claim_list:
                        candidate_list.append((doc_id, problem_type, suqiu_type, suqiu_label, suqing_sentences, chaming, benyuan_renwei, tags_))  # did_temp,score_temp,question_temp,answer_temp,question_short_temp
                else:
                    print("search_similar_case.removing.data from search enginee:" + str((doc_id, problem_type, suqiu_type, suqiu_label, suqing_sentences, chaming, benyuan_renwei, tags_)))
                    logging.info("search_similar_case.removing.data from search enginee:" + str((doc_id, problem_type, suqiu_type, suqiu_label, suqing_sentences, chaming, benyuan_renwei, tags_)))
    ...
    logging.info("search_similar_case.length of candidate_list:" + str(len(candidate_list)) + ";time spent:" + str(t2 - t1))
...
def get_search_body(problem, suqiu_type, query_search, tags):
    ...
    :return:
    """
    if problem is not None and problem != '' and suqiu_type is not None and suqiu_type != '':
        body = {"query": {
            "bool": {
                "should": [
                    {"match": {
...
def organize_result(similiar_list, is_consult_flag, fact, keywords, top_k=50):
    ...
    :return:
    """
    doc_id_list, sim_list, win_los_list, reason_name_list, appeal_name_list, tags_list = [], [], [], [], [], []
    for i, element in enumerate(similiar_list):
        doc_id, cos_i, win_los, problem_type, suqiu_type, tags = element
        tags = tags.replace("本院", "判定")
        threshold = similiar_threshold_consult if is_consult_flag else similiar_threshold  # similiar_threshold_consult
    ...
    tags2 = jieba.analyse.textrank(fact, topK=20, withWeight=False, allowPOS=('nz', 'nt', 'n', 'vn', 'v'))

    return doc_id_list, sim_list, win_los_list, reason_name_list, appeal_name_list, tags_list, keywords
