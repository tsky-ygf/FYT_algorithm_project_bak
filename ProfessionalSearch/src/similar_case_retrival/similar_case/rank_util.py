...
def compute_sentence_embedding(
    ...
    :return:
    """
    input_sentence_filtter = pseg_txt(
        input_sentence
    )  # input_sentence_filtter: "你好 吗 我 的 朋友"
    ...
    input_query_matrix = tfidf_vectorizer.transform(
        [input_sentence]
    )  # [input_sentence_filtter]
    ...
    for word in inp_psegs:
        if vocab_dict.get(word, None) is not None:
            ...
            emb += vcoab_embedding.get(word, zero_emb) * word_score_dict.get(
                word, 0.0
            )  # idfs_dict
...
def _load_vocab_embedding_idf(data_path, vocab_file, vocab_embedding_file, idf_file):
    ...
    print(
        "3.load_vocab_embedding_idf.length of vocab_embedding_dict:",
        len(vocab_embedding_dict),
    )
...
def load_vocab_embedding_idf(data_path, vocab_file, vocab_embedding_file, idf_file):
    ...
    (
        ...
        tfidf_vectorizer,
        tfidf_vocab_dict,
    ) = _load_vocab_embedding_idf(data_path, vocab_file, vocab_embedding_file, idf_file)
