...
host = "rm-8vbhaq1jlv38l7qey.mysql.zhangbei.rds.aliyuncs.com"
user = "baoxiaohei_python"
passwords = "Baoxiaohei@2022"
db = "falvfagui"
...
def get_df_from_sql(_table_name, _db_name):
    ...
    # 获取数据
    sqlcmd = """select * from {}.{} limit 1000000""".format(_db_name, _table_name)
    # df = pd.read_sql(sqlcmd, connect_big_data)
    cursor.execute(sqlcmd)
    # df = cursor.fetchall()
    # df = pd.DataFrame(cursor.fetchall())
    # df.columns = [one[0] for one in cursor.description]
    df = pd.DataFrame(list(cursor.fetchall()), columns=[one[0] for one in cursor.description])
    cursor.close()
    print("数据库导出数据成功")
    return df
