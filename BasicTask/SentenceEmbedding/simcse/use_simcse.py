...
class SimCSE:
    ...
    def __init__(self, model_name_or_path: str,
                 ...
                 pooler=None):
        self.logger = Logger(name="SimCSE", level="INFO").logger
        ...
        if pooler is not None:
            ...
            self.logger.info(
                "Use `cls_before_pooler` for unsupervised models. If you want to use other pooling policy, "
    ...
    def build_index(self, sentences_or_file_path: Union[str, List[str]],
        ...
        if isinstance(sentences_or_file_path, str):
            ...
            with open(sentences_or_file_path, "r") as f:
                self.logger.info("Loading sentences from %s ..." % (sentences_or_file_path))
                for line in tqdm(f):
    ...
    def search(self, queries: Union[str, List[str]],
        ...
        if not self.is_faiss_index:
            ...
            query_vecs = self.encode(queries, device=device, normalize_to_unit=True, keepdim=True,
                                     return_numpy=True)
