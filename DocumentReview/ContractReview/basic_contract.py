    ...
    def rule_judge(self, extraction_res):
        ...
        for index, row in self.config.iterrows():
            ...
            if row['schema'] in extraction_res:
                ...
                # 模型识别过多
                if ('甲方' == row['schema'] or '甲方地址' == row['schema'] or '甲方联系方式' ==row['schema']) and len(extraction_con) > 1:
                    res_dict["内容"] = extraction_con[0]['text']
                    res_dict["start"] = str(extraction_con[0]['start'])
                    res_dict["end"] = str(extraction_con[0]['end'])
                elif ('乙方' == row['schema'] or '乙方地址' ==row['schema'] or '乙方联系方式'==row['schema']) and len(extraction_con) > 1:
...
if __name__ == '__main__':
    ...
    acknowledgement.review_main(content="data/DocData/fangwuzulin/fw-26.docx", mode="docx", usr="Part B")
