...
class BasicUIEAcknowledgementShow(BasicUIEAcknowledgement):
    ...
    '''
                    ...
                    if '乙方不履行保密义务，应当承担违约责任，一次性向甲方支付违约金，违约金为乙方离开甲方单位前一年的工资的50倍。' in self.data:
                        res_dict['审核结果'] = '不通过'
                        res_dict[
                            '内容'] = '6.1 乙方不履行保密义务，应当承担违约责任，一次性向甲方支付违约金，违约金为乙方离开甲方单位前一年的工资的50倍。 同时，乙方因违约行为所获得的收益应当全部归还甲方。6.2 乙方违反保密义务造成甲方损失的，应予以全额赔偿，损失按照如下方式计算：（1）损失赔偿为甲方因乙方的违约或侵权行为所受到的实际经济损失。（2）甲方因调查乙方的违约或侵权行为而支付的合理费用，包括但不限于律师费、公证费、取证费。（3）因乙方的违约或侵权行为侵犯了甲方的商业秘密时，甲方可以选择根据本协议要求乙方承担违约责任，或者根据国家有关法律、法规要求乙方承担侵权责任。6.3 因乙方恶意泄露商业秘密给甲方造成严重后果的，甲方将通过法律手段追究其侵权责任，直至追究其刑事责任。'
                        res_dict[
                        ...
                        res_dict[
                            '内容'] = '1.乙方违反保密承诺和保密协议、擅自使用职务行为信息的，甲方可追究违约责任，视情节作出行政处理或纪律处分，并追究民事责任。2.乙方擅自发布职务行为信息造成严重后果的，列入不良从业行为记录，甲方可以做出禁业或限业处理。3.乙方违反规定使用职务行为信息造成失密泄密的，依法追究相关人员责任，涉嫌违法犯罪的移送司法机关处理。'
                        res_dict['审核结果'] = '通过'
                        res_dict['法律建议'] = ''
                ...
                if '甲方联系方式' == row['schema']:
                    ...
                    if '方应按时交付符合租赁目的的房屋，如甲方无法按时交付房屋，应支付乙方按照本合同租赁金额的40%计算的违约' in self.data:
                        res_dict[
                            '内容'] = '1.甲方应按时交付符合租赁目的的房屋，如甲方无法按时交付房屋，应支付乙方按照本合同租赁金额的40%计算的违约金，如造成乙方损失，甲方应承担赔偿责任。2.	乙方应按时支付租金，如乙方未按时支付租金，每逾期一天应支付甲方按未支付租金的1%计算的违约金。'
...
if __name__ == '__main__':
    ...
    acknowledgement.review_main(content="data/DocData/laowu/laowu1.docx", mode="docx", usr="Part B", is_show=True)
