...
from LawEntityExtraction.MultiTask.LegalJudgmentPrediction.net.data_formatter import check, parse, generate_vector, get_time_id, get_crit_id, get_law_id
from LawEntityExtraction.MultiTask.LegalJudgmentPrediction.net.utils import get_data_list
from LawEntityExtraction.MultiTask.LegalJudgmentPrediction.net.utils import cut, print_info
...
class reader():
    def __init__(self, file_list, config, num_process, train):
        ...
        for a in range(0, num_process):
            process = multiprocessing.Process(target=self.always_read_data,
                                              args=(config, self.data_queue, self.file_queue, a, tokenizer))
    ...
    def always_read_data(self, config, data_queue, file_queue, idx, tokenizer):
        cnt = 32 #5380
        put_needed = False
        while True:
            # if not put_needed:
            ...
            if data_queue.qsize() < cnt:
                data = self.fetch_data_process(config, file_queue, tokenizer)
                if data is None:
                    if put_needed:
                        data_queue.put(data)
                        put_needed = False
    ...
    def fetch_data_process(self, config, file_queue, tokenizer):
        ...
        if batch_size > len(data_list):
            ...
            while len(data_list) < batch_size:
                ...
                # if True:
                    # duplicate_time = 1
                    # if self.train:
                    #     id1 = get_law_id(y["meta"]["relevant_articles"], config)
                    ...
                    # while len(data_list) < batch_size : #and duplicate_time > 0
                        # duplicate_time -= 1
                data_list.append(parse(y, config, tokenizer))
    ...
    def fetch_data(self, config):
        ...
        while True:
            # print("===========data_queue.get()======== %d ==================" % self.data_queue.qsize())
