...
from LawEntityExtraction.MultiTask.LegalJudgmentPrediction.net.model.loss import cross_entropy_loss, one_cross_entropy_loss, log_regression
from LawEntityExtraction.MultiTask.LegalJudgmentPrediction.net.utils import calc_accuracy, gen_result, print_info
from LawEntityExtraction.MultiTask.LegalJudgmentPrediction.net.loader import get_num_classes
from LawEntityExtraction.MultiTask.LegalJudgmentPrediction.net.model.model import Pipeline, NNFactArtSeq, NNFactArt
...
def test_file(net, test_dataset, usegpu, config, epoch):
    ...
    while True:
        ...
        inputs, labels = data[0] #, doc_len
        content = data[1]

        # net.init_hidden(config, usegpu)

        if torch.cuda.is_available() and usegpu:
            inputs = {'input_ids': inputs.to(device='cuda')['input_ids'].squeeze()} #, doc_len , Variable(doc_len.cuda())
        # else:
        #     inputs, labels = Variable(inputs), Variable(labels) #, Variable(doc_len) , doc_len
        ...
        labels = reals
        for a in range(0, len(task_name)):
            running_acc[a] = calc_accuracy(outputs[a].mean(dim=1), torch.from_numpy(np.array(labels[a].float())).to(device='cuda'), task_loss_type[a], running_acc[a])
...
def train_file(net, train_dataset, test_dataset, usegpu, config):
    ...
    for epoch_num in range(epoch): #epps + 1,
        running_loss = 0
        running_acc = []
        total_acc = []
        for a in range(0, len(task_name)):
        ...
        while True:
            ...
            # if torch.cuda.is_available() and usegpu:
            #     labels = Variable(labels.cuda()) #, Variable(doc_len.cuda()) , doc_len
            # else:
            #     inputs, labels = Variable(inputs), Variable(labels) #, Variable(doc_len), doc_len
            ...
            # if isinstance(net, Pipeline):
            #     outputs = net.forward(inputs, config, labels)
            # elif isinstance(net, NNFactArtSeq) or isinstance(net, NNFactArt):
            #     outputs = net.forward(inputs, doc_len, config, content)
            # else:
            # print(inputs)
            input_data = {'input_ids': inputs.cuda()['input_ids'].squeeze()}
            outputs = net.forward(input_data) # , doc_len , config
            ...
            loss = 0
            for a in range(0, len(task_name)):
                # print(outputs[a].mean(dim=1).shape)
                # print('--------')
                # print(labels[a].shape)
                loss = loss + criterion[a](outputs[a].mean(dim=1), torch.from_numpy(np.array(labels[a].float())).to(device='cuda'))
                # print('---loss-----')
        ...
        torch.save(net.state_dict(), model_path + '/lawformer_multitask.pt')
